...
"""RSL-RL 策略回放入口（Go2 任务）。

用于加载策略检查点并在仿真中运行，可选加载动力学模型。
"""
...
def main(
    ...
    if agent_cfg.class_name == "OnPolicyRunner":
        ...
        runner = MBPOOnPolicyRunner(env, agent_cfg.to_dict(), log_dir=None, device=agent_cfg.device)
    else:
    ...
    timestep = 0
    if agent_cfg.class_name == "MBPOOnPolicyRunner" and hasattr(env.unwrapped, "init_imagination_history"):
